...
esp_err_t servo_dog_ctrl_init(servo_dog_ctrl_config_t *config)
{
    ...
    g_servo_dog->state = DOG_STATE_IDLE;
...
{
    if (g_servo_dog == NULL) {
        return DOG_STATE_IDLE;
    }
    return g_servo_dog->state;
}

const char *servo_dog_ctrl_get_state_name(void)
{
    servo_dog_state_t state = servo_dog_ctrl_get_state();
    if (state >= DOG_STATE_MAX) {
        return "UNKNOWN";
    }
    return g_servo_dog_action_table_name[state];
}
