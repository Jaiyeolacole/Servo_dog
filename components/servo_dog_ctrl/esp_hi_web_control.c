...
// API: GET /status
static esp_err_t status_handler_func(httpd_req_t *req)
{
    char response[160];
    int len = snprintf(response, sizeof(response),
                        "{\"state\":%d,\"name\":\"%s\",\"calibrating\":%s}",
                        (int)servo_dog_ctrl_get_state(),
                        servo_dog_ctrl_get_state_name(),
                        is_calibration_mode ? "true" : "false");
