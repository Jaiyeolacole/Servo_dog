...
/**
 * @brief Get the servo dog's current action state
 *
 * @return The state enum value currently being executed (or DOG_STATE_IDLE if not initialized)
 */
servo_dog_state_t servo_dog_ctrl_get_state(void);

/**
 * @brief Get a human-readable name for the servo dog's current action state
 *
 * @return Pointer to a static string naming the current state (e.g. "DOG_STATE_FORWARD")
 */
const char *servo_dog_ctrl_get_state_name(void);
